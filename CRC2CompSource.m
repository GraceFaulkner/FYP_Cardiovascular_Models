%% CRC Windkessel (Rod's current option) with Coupled LV and resistive valves 
...
global C_a_h C_v_h R_h C_a_l C_v_l R_l Z R L rmv rav HR tauS1 tauS2 ELVmax ELVmin t1 t2 t3 t4 grad_l grad_h
...
grad_h      = UBPP/(t2-t1);
tspan       = [0 T_max];
% IV          = mcfp*(1/ELVmin + C_art + C_ven);                             % Initial volume
...
V_tot   = V_lv + V_a_h + V_v_h + V_a_l + V_v_l;
% 
% %plots
% figure('name', 'source')
% plot(t, P_static) 
...
figure('name','signals1'); hold on; grid on; 
    plot(t,P(:,1)); plot(t,P(:,2)); plot(t,P(:,3)); %plot(t,P_abs_h);% plot(t,P_abs_l); 
    xlabel('t (secs)'); ylabel('p(t) (mmHg)'); legend('P_L_V','P_A_O','P_V_E_N')%,'P_A_B_S_h','P_A_B_S_l'); title('Pressures')
...
figure('name', 'volumes');hold on; grid on;
    ...
    legend('V_L_V','V_A_O_h','V_V_E_N_h','V_A_O_l','V_V_E_N_l', 'V_T_O_T');
    title('Volumes')
...
iAV = (P1 - P2)/rav .* (P1>P2);
iMV = (P3 - P1)/rmv .* (P3>P1);
figure('name', 'currents');hold on; grid on;
    plot(t, iAV);
    plot(t, iMV);
    xlabel('t (secs)'); ylabel('I(t) (m^2/sec)');
    legend('i_A_V', 'i_M_V')
    title('Currents')
...
global C_a_h C_v_h R_h C_a_l C_v_l R_l Z R L rmv rav HR t1 t2 t3 t4 grad_l grad_h 
t=t
[Elocal, dEdtlocal] =  elastance2 (t);
iAV       = valve(P(1)-P(2),rav);
iMV       = valve(P(3)-P(1),rmv);         
sourceh   = ( heaviside(t-t1   ) - heaviside(t-t2   ) ) * grad_h +...
          - ( heaviside(t-t3) - heaviside(t-t4) ) * grad_h; 
sourcel   = ( heaviside(t-t1   ) - heaviside(t-t2   ) ) * grad_l +...
          - ( heaviside(t-t3) - heaviside(t-t4) ) * grad_l; 
dPdt(1)   = (iMV-iAV)*Elocal + P(1)/Elocal*dEdtlocal;                      % LV 
dPdt(2)   = (1/(C_a_h + C_a_l))*(iAV...                                    % Artery pressure
          - (1/R_h + 1/ R_l)*P(2) + (1/R_h + 1/R_l)*P(3));
dPdt(3)   = (1/(C_v_h + C_v_l))*(-iMV...                                    % Venous pressure
          + (1/R_h + 1/ R_l)*P(2) - (1/R_h + 1/R_l)*P(3)...
          + C_v_l*sourcel + C_v_h*sourceh);
% No distal capacitance case 
% dPdt(2)   = iAV/C_art - iMV/C_art;
% dPdt(3)   = R/(rmv+R)*dPdt(1) + rmv/(rmv+R)*dPdt(2);
dPdt      = dPdt';
end
