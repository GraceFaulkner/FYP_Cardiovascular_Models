% Compare results for values of Vmax
close all
clear all
...
%Model parameters
global C_v_h C_v_l C_v_t C_a_tot R_tot rmv rav HR tauS1 tauS2 ELVmax ELVmin t1 t2 t3 t4 grad_l grad_h grad_t C0 Vmax 
...
Vactual      = V0 + alpha .* atan(C0./alpha .* Ptrans);
Ptrans2      = [min(Ptrans):0.1:max(Ptrans)];
Vtheoretical = V0 + alpha .* atan(C0./alpha .* Ptrans2);

figure('name', 'volumes'); hold on; grid on;
    plot(Ptrans,  Vactual)
    plot(Ptrans2, Vtheoretical)
    xlabel('Pressure (mmHg)')
    ylabel('Volume ( ml)')
    legend('Vactual', 'Vtheoretical')
    title('Non-linear Volume Comparison')
...
global C_v_h C_v_l C_v_t C_a_tot R_tot rmv rav t1 t2 t3 t4 grad_l grad_h grad_t C0 Vmax
t=t
...
alpha     = 2* Vmax / pi;
Ptrans    = P(3) - sourcet;
theta     = 2* C0^2 /alpha * Ptrans^2 * (1+ (C0/alpha * Ptrans)^2)^(-2);
% Vx        = 1500;
% gamma     = Vx/Vmax^2 * C0^2 *pi *Ptrans*(1+(pi*C0/2*Vmax*Ptrans)^2)^(-2);
